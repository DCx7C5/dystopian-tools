...
backup_and_rename() {
    ...
    if ! jq --arg idx "$index" \
      ...
      '.ssl.keys[$idx][$backup_key] = .ssl.keys[$idx][$file_type] |
       del(.ssl.keys[$idx][$file_type])' -- "$DC_DB" > "${DC_DB}.tmp"; then
    ...
    if ! jq --arg idx "$index" \
      --arg backup_path "$(get_realpath_from_file "$backup_file")" \
      --arg backup_key "$backup_key" \
      '.ssl.keys[$idx][$backup_key] = $backup_path'  -- "$DC_DB" > "${DC_DB}.tmp"; then
...
add_to_ssl_keys_database() {
    ...
    if ! jq --arg idx "$1" \
      --arg key "$2" \
      --arg value "$3" \
      '.ssl.keys[$idx][$key] = $value' -- "$DC_DB" > "${DC_DB}.tmp"; then
...
add_to_ca_database() {
    ...
    if ! jq --arg type "$storage_type" \
            ...
            --arg key "$key" \
            --arg val "$value" \
            '.ssl.ca[$type][$idx][$key] = $val' -- "$DC_DB" > "${DC_DB}.tmp"; then
...
add_gpg_key() {
    if ! jq --arg idx "$1" \
        '.gpg.keys[$idx] = {}' -- "$DC_DB" > "${DC_DB}.tmp"; then
...
add_gpg_sub() {
    if ! jq --arg idx "$1" \
            --arg sub "$2" \
            '.gpg.keys[$idx].subkeys[$sub] = {}' -- "$DC_DB" > "${DC_DB}.tmp"; then
        echoe "Failed to update GPG database for gpg: $1: $2 : $3"
        rm -f -- "${DC_DB}.tmp"
        return 1
    fi
...
add_to_gpg_key() {
    if [ $# -eq 4 ]; then
        if ! jq -r \
                ...
                --arg key "$3" \
                --arg val "$4" \
                '.gpg.keys[$idx].subkeys[$sidx][$key] = $val' -- "$DC_DB" > "${DC_DB}.tmp"; then
        ...
        if ! jq -r --arg idx "$1" \
                    --arg key "$2" \
                    --arg val "$3" \
                    '.gpg.keys[$idx][$key] = $val' -- "$DC_DB" > "${DC_DB}.tmp"; then
                echoe "Failed to update GPG database for gpg: $1: $2 : $3"
                rm -f -- "${DC_DB}.tmp"
                return 1
        fi
...
cleanup_ca_index() {
    ...
    if ! jq -e --arg type "$ca_type" \
            --arg idx "$ca_index" \
            'del(.ssl.ca[$type][$idx])' -- "$DC_DB" > "${DC_DB}.tmp"; then
