...
delete_key_from_keys_index() {
    ...
    jq -r --arg idx "$index" --arg key "$key" 'del(.ssl.keys[$idx][$key])' -- "$DC_DB" > "$temp_file" || {
        echoe "Failed deleting keys from $index"
        rm -f -- "$temp_file"
        return 1
    }
...
delete_key_from_ca_index() {
    ...
    jq -r --arg idx "$index" --arg key "$key" 'del(.ssl.ca.root[$idx][$key]) // del(.ssl.ca.intermediate[$idx][$key])' -- "$DC_DB" > "$temp_file" || {
...
cleanup_index() {
    ...
    jq --arg idx "$index" 'del(.ssl.keys[$idx])' -- "$DC_DB" > "$temp_file" || {
...
backup_and_rename() {
    ...
    if ! jq --arg idx "$index" \
      ...
      '.ssl.keys[$idx][$backup_key] = .ssl.keys[$idx][$file_type] |
       del(.ssl.keys[$idx][$file_type])' -- "$DC_DB" > "$temp_file"; then
    ...
    if ! jq --arg idx "$index" \
      --arg backup_path "$(get_realpath_from_file "$backup_file")" \
      --arg backup_key "$backup_key" \
      '.ssl.keys[$idx][$backup_key] = $backup_path'  -- "$DC_DB" > "$temp_file"; then
...
add_to_ssl_keys_database() {
    ...
    if ! jq --arg idx "$1" \
      --arg key "$2" \
      --arg value "$3" \
      '.ssl.keys[$idx][$key] = $value' -- "$DC_DB" > "$temp_file"; then
...
add_to_ca_database() {
    ...
    if ! jq --arg type "$storage_type" \
            ...
            --arg key "$key" \
            --arg val "$value" \
            '.ssl.ca[$type][$idx][$key] = $val' -- "$DC_DB" > "$temp_file"; then
...
cleanup_ca_index() {
    ...
    if ! jq -e --arg type "$ca_type" \
            --arg idx "$ca_index" \
            'del(.ssl.ca[$type][$idx])' -- "$DC_DB" > "$temp_file"; then
